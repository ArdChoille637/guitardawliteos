...
- [ ] **1.15 `[SPIKE C]` Display-only + controls bring-up** (M) — *(re-scoped 2026-07-23: touch DROPPED — UI = 5 rotary encoders + sustain pedal + plain display)*. Build `sample/28-touchscreen` with `DSI_DISPLAY=0` and **`bEnableTouch=FALSE`** on the Touch Display 2 as a display-only panel (GT911 never powered; zero 40-pin GPIOs holds), then `addon/lvgl/sample` with **90° software rotation** and **LVGL encoder-group navigation**. Plus the input spike: 5× EC11/KY-040 quadrature + pedal read via one `CGPIOPin::ReadAll()` at a 2 kHz core-3 tick (transition-table decode, Schmitt on, boot auto-polarity for the pedal — [retro-deck-design.md](../docs/retro-deck-design.md) §1/§1b). Gates the faceplate cutout. Fallback: plain HDMI (already proven live).
...
**Full electrical review + wiring diagram done:** [docs/hardware-review.md](../docs/hardware-review.md), [docs/wiring-diagram.svg](../docs/wiring-diagram.svg). Key additions beyond the pin map below: PCM1808 needs a **5 V analog rail** (not 3.3 V), a **guitar front-end** (buffer → ×3 gain → bias-to-VBIAS mid-rail → AA) is mandatory, op-amp runs on 5 V, no level shifters. Analog rail off header 5 V (**never** header 3V3 *for analog*); ADC digital VDD from header **pin 1 direct** — the CJMCU breakout has no onboard regulator and the "local LDO" applies only to the KiCad bare-chip board. **For the permanent soldered build see [solder-build.md](../docs/solder-build.md), which freezes every open choice.** **Buildable schematic** ([docs/schematic.md](../docs/schematic.md), SPICE-verified + ERC-reviewed), **KiCad netlist + symbols** ([kicad/](../kicad/), bare-chip, ERC-checked), and a **breadboard build guide** ([docs/breadboard-build.md](../docs/breadboard-build.md)) are done.
...
- [ ] **6.6 Controls input handler** (M) — 5× rotary encoders (A/B/SW) + sustain-pedal footswitch: one `ReadAll()` bank read per 2 kHz core-3 tick, quadrature transition table + 10 ms switch debounce, SPSC event queue to the UI; pedal polarity auto-calibrated at boot (Yamaha FC-series is normally-closed — [retro-deck-design.md](../docs/retro-deck-design.md) §1b). Map: knobs = trim/level/loop-select/feedback/menu (draft), pedal = record/overdub tap · double-tap stop · hold clear.
