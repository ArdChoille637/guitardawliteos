...
#### ⛔ Update 2026-07-10 — E2's "async-but-fs-locked" reading was wrong; topology pivoted to ADC-as-master
E2 ("MCLK↔BCLK phase not guaranteed but OK") is true for **phase**, but was stretched to cover **frequency**: "fs-locked" is unachievable between two free-running crystals. With the Nano generating SCKI and the Pi generating BCK/LRCK independently, they drift tens of ppm apart; the PCM1808's clock-halt/resync logic (datasheet §7.4.2 — LRCK-vs-SCKI sync enforcement, mute + ~1 ms fade-in on every resync) would interrupt audio every fraction of a second. Corroborated by an RPi engineer (forum user 6by9) calling independently-clocked SCKI "a non-starter", and by the community-standard Pi wiring for this chip: **PCM1808 in MASTER mode with the external oscillator, Pi as I²S slave**.
**Effect on plan:** strap **MD0=MD1=HIGH** (master, 256 fs → SCKI/256 = 48 kHz; FMT=GND stays I²S); the ADC drives BCK/LRCK, the Pi consumes them (`bSlave=TRUE` → Circle's RP1 **I2S1** instance, same GPIO18-21 pads via AltFn4 — slave is a first-class Circle path, verified in `i2ssoundbasedevice-rp1.cpp` RunI2S). The PCM5102A is indifferent (BCK-PLL). Whole system is now one clock domain rooted at the Nano's crystal (absolute rate error ~±10 ppm — musically irrelevant for a self-contained looper). Kernel flipped via `GDAW_I2S_SLAVE` in src/config.h. Bench consequence: with no ADC wired there are no bus clocks, so the engine produces no callbacks — `cap`/`play` frozen at 0 on a bare-board boot is expected, not a fault. Sources: TI SLES177B §7.3.5/§7.4.2; raspberrypi.com forums (PCM1808 master-mode wiring, 6by9 comments); WLED MoonModules line-in guide; Circle `lib/sound/i2ssoundbasedevice-rp1.cpp`.
